...
__global__ void kernel_call(int N, int B, float *in, float *out) {
    ...
    /* MY CHANGE: get the pointer to the tile for the block -> same logic from banks_multi_kernel but for blcks*/
    size_t num_elem_in_tile = N*N;
    float* tile_input = in + num_elem_in_tile * ((blockIdx.x / B) * B + (blockIdx.x % B)); // input = row * B + col 
    /* transpose output ptr*/
    float* tile_output = out + num_elem_in_tile * ((blockIdx.x % B) * B + (blockIdx.x / B)); // out = col * B + row 
    ...
    for (int i = 0; i != 64 * 64 / blockDim.x; ++i) {
        ...
        share_buf[transpose_row * 64 + stagger_col] = tile_input[seq_access]; /* put the curr value into shared buffer*/
    }
    ...
    for (int i = 0; i != 64 * 64 / blockDim.x; ++i) {
        ...
        tile_output[seq_access] = share_buf[transpose_row * 64 + stagger_col];
    }
...
int main() {
    ...
    for (int i = 0; i < 1000; i++){
        ...
        */
        /* MY CHANGE: put all blocks in. */
        kernel_call<<<(B * B), 128>>>(N, B, dev_in, dev_out);
