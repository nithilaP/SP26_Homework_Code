...
def overlap_and_merge_implementation(input_image, image_name, device):
    ...
    while (starting_x <= image_width - patch_width):
        while (starting_y <= image_height - patch_height):
            ...
            starting_y += patch_height // 3 # move over 1/3 of a patch for the next patch
        starting_y = 0 # reset y once all vertical patches on that x axis are done
        starting_x += patch_width // 3 # move over 1/3 of a patch height for the next patch
    ...
    for i, found_object in enumerate(dog_cat_found):
        ...
        for j in range(i+1, len(dog_cat_found)):
            ...
            if (distance < patch_width * 1.0):
                ...
                # keep track of the best score for the final merge scores.
    ...
    # redo the detection on the newly merged boxes
...
if __name__ == "__main__":
    ...
    # Documentation for process: Did this to determine indices of dogs and cats in the category.
