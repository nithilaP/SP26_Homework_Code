...
# AlexNet-Inspired Implementation
class AlexNet_Inspired_CNN(nn.Module):
...
if __name__ == "__main__":
    ...
    # baseline CNN 
    my_cnn = AlexNet_Inspired_CNN(num_output_classes=100)
    my_cnn, train_loss, train_accuracy, test_loss, test_accuracy = train(net=my_cnn, num_epoch=epochs, learning_rate=learning_rate, momentum=momentum, weight_decay=weight_decay, 
    ...
    generate_plots("AlexNet_Inspired_CNN", epochs_axis, train_loss=train_loss, train_accuracy=train_accuracy, test_loss=test_loss, test_accuracy=test_accuracy)
    visualize_preds(my_cnn, "AlexNet_Inspired_CNN", test_data, train_data.classes, device)
